...
class Weapon(object):
	...
	def __init__(self, typ, random):
		...
		weaponModifiers = {'HersheyKisses': [1, 1, 100,000], 'SourStraws': [1, 1.75, 2],
							 'ChocolateBars': [2, 2.4, 4], 'NerdBombs': [3.5, 5, 1]}
		
		if (self.weaponType != 'HersheyKisses'):
			self.Modifier = uniform(weaponModifiers[self.weaponType][0], 
										weaponModifiers[self.weaponType][1])
		else:
			self.Modifier = 1
			self.uses = weaponModifiers[self.weaponType][2]
	
	####################################################################
	#Method that reduces the amount of times the weapon is used.
	####################################################################	
	def useWeapon():
		uses = uses - 1
